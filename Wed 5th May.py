...
def mainline(choose):
    ...
    while choose == 't':
        ...
        Menu = [    [sg.Text("Welcome to the Rust Manager")],
                    [sg.Button("1"), sg.Text("Signup")],
                    [sg.Button("2"), sg.Text("Login")],
                    [sg.Button("3"), sg.Text("Edit Favourites")],
                    [sg.Button("4"), sg.Text("ADMIN Update Prices (No GUI)")],   
                ]
            ...
            if events == '1':
                ...
                if UserID != 'None':
                    ...
                    while bigloop == 0:
                        ...
                        [sg.Button("1"), sg.Text("View Favourites ")],
                        [sg.Button("2"), sg.Text("Add Favourites")],
                        [sg.Button("3"), sg.Text("Remove Favourites")],
...
def View_favs(connection,UserID):
    with connection.cursor() as cursor:
        sql = "SELECT `skin` from `userfav` where `user` = '{UserID}'".format(UserID=UserID)
        test = cursor.execute(sql)
        rows = cursor.fetchall()
        skins = []
        for a in rows:
        ...
        for a in skins:
            data = []
            sql ="SELECT `IconURL`,`Price`,`Listings` from `rustskins` where `skinname` = '{skin}'".format(skin=a)
            cursor.execute(sql)
            b = cursor.fetchall()
            for c in b:
                Icons.append(str(c['IconURL']))
                price.append(int(c['Price']))
                Listings.append(int(c['Listings']))
        
    #path = input("Input path for image overwriting: ")
    #/Users/fynnmallon/OneDrive - St Augustines College/SDD/Photos/1.jpg
    path ='Users/fynnmallon/OneDrive - St Augustines College/SDD/Photos/1.png'
    icon_tag = 'https://community.akamai.steamstatic.com/economy/image/'
    for a in range(0, len(skins)):
        recieve = requests.get(icon_tag+Icons[a])
        recieve = recieve.content
        #f = open(path, 'wb')
        #f.write(recieve)
        #f.close()
        b = base64.b64encode(recieve)
        #img = Image.open(io.BytesIO(recieve))
        sg.ChangeLookAndFeel('LightGrey1')   # Design Colour
        # Design of login screen
        cost = price[a]/100
        cost = str(cost)
        fav = [  [sg.Text('View Favourites')],
                        [sg.Text("Skin Name: "), sg.Text(skins[a])],
                        [sg.Text("Price: "), sg.Text("$" +cost)],
                        [sg.Text("Market Listings: "), sg.Text(Listings[a])],
                        [sg.Image(data=b, key='__IMAGE__',)],
                        [sg.Button('Next'),]]
        window = sg.Window('Favourite Managed',fav)
        look = True
        while look == True:
...
def remove_favs(connection,UserID):
    with connection.cursor() as cursor:
        sql = "SELECT `skin` from `userfav` where `user` = '{UserID}'".format(UserID=UserID)
        rows = cursor.fetchall()
        skins = []
        ...
        if event == 'Remove':
            ...
            with connection.cursor() as cursor:
                    # Create a new record if all above criteria is met
                    sql = "DELETE from `userfav` where `skin`= '{skin}' AND `user` = '{UserID}'".format(skin = skin, UserID = UserID)
                    cursor.execute(sql)
            connection.commit() #commits to database
            window.close()
        else:
